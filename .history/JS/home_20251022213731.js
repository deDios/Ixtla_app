  ...
  if (img) {
    ...
    if (window.gcSetAvatarSrc) {
      ...
      const candidates = idu
        ? [
            `/ASSETS/usuario/usuarioImg/img${idu}.png`,
            `/ASSETS/usuario/usuarioImg/img${idu}.jpg`,
          ]
        : [];
      let i = 0;
      const tryNext = () => {
...
function filterRoleVisibility(items, { isAdmin, isPres, isDir, soyPL, isJefe, isAnal }) {
  if (isAdmin || isPres) return items; // ellos ven todo
  const hide = new Set(["asignacion", "solicitud"]);
  if (isDir || soyPL || isJefe || isAnal) {
    return items.filter(r => !hide.has(String(r?.estatus_key || r?.estatus || "").toLowerCase()) &&
                              !hide.has(String(r?.estatus?.key || "").toLowerCase()));
  }
  return items; // resto: ya vienen acotados por fuente
}
