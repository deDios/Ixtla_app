...
(function () {
  ...
  const ReqActions = (() => {
    ...
    function setStatusAndRefresh(next, cause=""){
      const wrapper = loadDemo();
      const data = wrapper.data;
      const prev = Number(data.estatus||0);
      data.estatus = next;
      saveDemo(data);
      console.log("[ReqActions] status", { prev, next, cause });
      paintStepper(next);
  ...
  function openEstadoModal({type,nextStatus}){
    ...
    console.log("[ModalEstado] open", _pendingAction);
  ...
  function closeEstadoModal(){ console.log("[ModalEstado] close"); modal.classList.remove("open"); modal.setAttribute("aria-hidden","true"); _pendingAction=null; }
  btnClose?.addEventListener("click", closeEstadoModal);
  modal?.addEventListener("click", (e)=>{ if(e.target===modal) closeEstadoModal(); });
  // export para ReqActions.ensureModalOrFallback
  window.openEstadoModal = openEstadoModal;

  form?.addEventListener("submit",(e)=>{
    e.preventDefault();
    console.log("[ModalEstado] submit", _pendingAction);
    ...
    // opcional registrar nota
    data.demoNotas = [...(data.demoNotas||[]), {tipo:_pendingAction.type, motivo, fecha:new Date().toISOString()}];
    saveDemo(data);
    console.log("[ModalEstado] status change", { prev, next:data.estatus, motivo });
