...
{
  error_log('[IXTLA_C_CAT_ESTATUS] ' . $message);
...
function consultar_estatus_por_id(mysqli $con, int $estatus_id): array
{
  $sql = "
        SELECT *
        FROM cat_estatus
  ...
  return estatus_row($row);
}

function consultar_estatus_por_codigo(mysqli $con, string $codigo): array
{
  $sql = "
        SELECT *
        FROM cat_estatus
...
function consultar_estatus(mysqli $con, array $in): array
{
  $q = str_clean($in, 'q');
...
try {
  ...
  if ($estatus_id && $estatus_id > 0) {
    $data = consultar_estatus_por_id($con, $estatus_id);
  ...
  if ($codigo !== '') {
    $data = consultar_estatus_por_codigo($con, $codigo);
  ...
  $result = consultar_estatus($con, $in);
  ...
  error_log('[IXTLA_C_CAT_ESTATUS][SQL] ' . $e->getMessage());

  json_response([
    "ok" => false,
    "error" => "Error al consultar estatus"
  ], 500);
} catch (Throwable $e) {
  if (isset($con) && $con instanceof mysqli) {
    try {
  ...
  error_log('[IXTLA_C_CAT_ESTATUS][ERROR] ' . $e->getMessage());
