...
(function () {
  ...
  log("Archivo cargado (hook de expediente listo para conectar).");
...
<body>
  ...
  /**
   * Lógica de generación (separada para poderla reutilizar).
   */
  ...
  /**
   * Delegación de click para el botón "Generar expediente".
   * Esto evita problemas si el botón se renderiza dinámicamente.
   */
  function initDelegatedClick() {
    document.addEventListener("click", (ev) => {
      const btn = ev.target.closest("#kb-btn-expediente");
      if (!btn) return;

      log("Click detectado en #kb-btn-expediente");
      handleGenerate();
    });

    log("Delegación de click para #kb-btn-expediente lista.");
  }

  if (document.readyState === "loading") {
    document.addEventListener("DOMContentLoaded", initDelegatedClick);
  } else {
    initDelegatedClick();
  }
})();