...
(() => {
  ...
  const timeMeta  = modal.querySelector("#ix-report-date"); // opcional

  // campos
  const inpNombre = modal.querySelector("#ix-nombre");
  const inpDom    = modal.querySelector("#ix-domicilio");
  let   inpCP     = modal.querySelector("#ix-cp");       // se asegura <select>
  let   inpCol    = modal.querySelector("#ix-colonia");  // se asegura <select>
  ...
  function validateForm(showErrors = true) {
    ...
    // asunto obligatorio SOLO si es “Otros”
    const isOtros = /\botr(os|o)\b/i.test(currentTitle);
    if (isOtros && inpAsunto) {
      const a = (inpAsunto.value || "").trim();
      if (!a) { ok = false; if (showErrors) setFieldError(inpAsunto, "Coloca un título para tu reporte."); }
      else setFieldError(inpAsunto);
    }
