...
<body>
  ...
  function openPrintWindow(html) {
    ...
    // Pequeño delay por si el navegador tarda en renderizar
    win.focus();
    try {
      win.print();
    } catch (e) {
  ...
  function onGenerateClick() {
    ...
    // Si de plano no hay nada (ej. folio vacío), no generamos.
    if (!folio || folio === "—") {
      toast(
  ...
  function initExpediente() {
    const btn = document.getElementById("kb-d-expediente");
    if (!btn) {
      warn("Botón #kb-d-expediente no encontrado en el DOM.");
      return;
    }
