(() => {
  ...
  // Comentarios de Pausa / Cancelación (CCP)
  const ENDPOINT_CCP = "https://ixtla-app.com/db/web/ixtla01_c_ccp.php";

  const FETCH_TIMEOUT = 12000;
  const IX_DEBUG_TRACK = true; // colocar false antes del sabado

  /* ============= Helpers ============= */
  function getDepIdFromURL() {
    try {
      const p = new URLSearchParams(location.search),
        n = Number(p.get("depId"));
  ...
  const log = (...a) => {
    if (IX_DEBUG_TRACK) console.log("[IX][track]", ...a);
  };
  const warn = (...a) => {
    if (IX_DEBUG_TRACK) console.warn("[IX][track]", ...a);
  };
  const err = (...a) => {
    if (IX_DEBUG_TRACK) console.error("[IX][track]", ...a);
  };

  function withTimeout(factory, ms = FETCH_TIMEOUT) {
    const c = new AbortController(),
      t0 = performance.now(),
      to = setTimeout(() => c.abort(), ms);
    return factory(c.signal)
      .then((r) => {
        clearTimeout(to);
  ...
  const steps = $$(root, ".ix-stepper .ix-step"),
    popBtns = $$(root, ".ix-stepper .ix-stepbtn"),
    popovers = $$(root, ".ix-stepper .ix-pop"),
  ...
  function showPanel(which) {
    ...
    if (which) {
      which.hidden = false;
      which.classList.add("is-visible");
    }
  }
  ...
  function statusKeyFromRow(row) {
    switch (Number(row?.estatus)) {
      ...
      case 3:
        return "proceso";
      case 4:
        return "pausado"; // no tiene status visible solo el mensaje abajo
      case 5:
        return "cancelado"; // lo mismo tambien
      case 6:
        return "finalizado";
      default:
        return "proceso";
  ...
  let refreshSubStatusMessageFromCCP = async function () {};
  ...
  function renderResult(row) {
    ...
    try {
      if (pResult) pResult.dataset.requerimientoId = String(row?.id || row?.requerimiento_id || "");
    } catch {}
    ...
    if (isSubStatus(key)) {
      ...
      if (Number.isFinite(reqId) && reqId > 0) {
        refreshSubStatusMessageFromCCP(reqId, key);
      }
    } else {
  ...
  async function handleSubmit(e) {
    ...
    try {
      ...
      const target = r2.json.data.find(
        (row) => String(row?.folio || "").toUpperCase() === folioNorm.toUpperCase()
      );
