...
(function () {
  ...
  window._rvHelpers = { $, $$, toast }; // usado por Planeación
  ...
  const relShort = (when) => {
    ...
    const s = Math.max(0, Math.floor((Date.now() - (Number.isFinite(t) ? t : Date.now())) / 1000));
  ...
  const ENDPOINTS = {
    ...
    DEPT_LIST:           "/DB/WEB/ixtla01_c_departamento.php", // case según prod
  ...
  /* ============================
   * Sesión (cookie ix_emp o Session.get)
   * ============================*/
  ...
  function getEmpleadoIdFromSession() {
    ...
    if (!empId) warn("[Sesion] sin empleado_id en sesión", s);
    else log("[Sesion] empleado_id:", empId);
    return empId;
  }
   ...
   * ============================*/
  const statusLabel = (s) => ({
    0: "Solicitud", 1: "Revisión", 2: "Asignación", 3: "Proceso",
    4: "Pausado",   5: "Cancelado", 6: "Finalizado",
  }[Number(s)] || "—");

  const statusBadgeClass = (s) => ({
    0: "is-muted", 1: "is-info", 2: "is-info", 3: "is-info",
    4: "is-warning", 5: "is-danger", 6: "is-success",
  }[Number(s)] || "is-info");

  function paintStepper(next) {
    $$(".step-menu li").forEach((li) => {
      const s = Number(li.dataset.status);
      li.classList.toggle("complete", s < next);
      li.classList.toggle("current",  s === next);
    });
  }
  ...
  function askMotivo(titulo = "Motivo") {
    return new Promise((resolve, reject) => {
      ...
      const onSubmit = (e) => { e.preventDefault(); const v = txt.value.trim(); if (!v) return txt.focus(); cleanup(); resolve(v); };
  ...
  async function updateReqStatus({ id, estatus, motivo }) {
    ...
    log("[API] REQUERIMIENTO_UPDATE payload:", body);
  ...
  async function onAction(act) {
    ...
    if (!id) return toast("No hay id de requerimiento en la URL", "danger");
    ...
    try {
      if (act === "start-revision") {
        ...
        if (!ok) return toast("Para iniciar proceso necesitas al menos un proceso y una tarea.", "warning");
  ...
  // Cache empleados (para nombres/avatars desde endpoint si aplica)
  ...
  async function getEmpleadoById(id) {
    ...
    const full = [nombre, apellidos].filter(Boolean).join(" ").trim() || (emp?.empleado_display || "").trim() || "—";
    const avatar = emp?.avatar_url || emp?.foto_url || emp?.foto || emp?.img || emp?.img_perfil || emp?.imagen_url || emp?.path_imagen || emp?.imagen || null;

    const info = { id: key, nombre: full, avatar: avatar ? (avatar.startsWith("/") ? avatar : `/${String(avatar).replace(/^\/+/, "")}`) : null };
    EMP_CACHE.set(key, info);
    return info;
  }

  /* ============================
   * Departamentos (lista + pick)
   * ============================*/
  async function getDeptByIdOrName({ id, nombre }) {
    ...
    let res;
    try {
      res = await postJSON(ENDPOINTS.DEPT_LIST, { page: 1, page_size: 200, status: 1 });
    } catch (e) {
      warn("[Dept] fetch lista error:", e);
    ...
    // Director
    let director = null;
    if (infoDept.director_id) { try { director = await getEmpleadoById(infoDept.director_id); } catch {} }
    if (!director) {
    ...
    if (infoDept.primera_linea_id) { try { primeraLinea = await getEmpleadoById(infoDept.primera_linea_id); } catch {} }
    if (!primeraLinea) {
  ...
  function paintContacto(req) {
    ...
    const set = (labelText, val) => {
      const row = Array.from(p.querySelectorAll(".exp-field")).find((r) =>
        (r.querySelector("label")?.textContent || "").trim().toLowerCase().startsWith(labelText.toLowerCase())
      );
  ...
  async function paintDetalles(req) {
    ...
    const put = (labelStartsWith, value, asLink = false) => {
      const row = Array.from(grid.querySelectorAll(".exp-field")).find((r) =>
        (r.querySelector("label")?.textContent || "").trim().toLowerCase().startsWith(labelStartsWith.toLowerCase())
      );
      const dd = row?.querySelector(".exp-val");
      if (!dd) return;
      if (asLink) {
  ...
  /* ============================
   * Comentarios (id_usuario → empleado_id fallback)
   * ============================*/
  ...
  const placeholderAvatar = () => "/ASSETS/user/img_user1.png";
  const srcNeedsBust = (src) => !src.includes("/ASSETS/user/img_user1.png");
  ...
  async function renderCommentsList(items = [], reqId) {
    ...
    for (const r of items) {
      ...
      // IDs para avatar: USUARIO primero → EMPLEADO fallback
      const usuarioId  =
        (Number(r.usuario_id) > 0 && Number(r.usuario_id)) ||
        (Number(r.created_by_user) > 0 && Number(r.created_by_user)) ||
        null;
      const empleadoId =
      ...
      console.log("[Comentarios][Autor/IDs]", {
        comentario_id: r.id,
        usuario_id_raw: r.usuario_id,
        created_by_user_raw: r.created_by_user,
        empleado_id_raw: r.empleado_id,
        created_by_raw: r.created_by,
        usuarioId_usado: usuarioId,
        empleadoId_usado: empleadoId,
        nombrePlano: nombre,
      });
      ...
      const localCandidates = [
        ...(usuarioId ? [
          `/ASSETS/user/userImgs/img_${usuarioId}.png${bust}`,
          `/ASSETS/user/userImgs/img_${usuarioId}.jpg${bust}`,
        ] : []),
        ...(empleadoId ? [
      ...
      // URL desde endpoint empleado (si existe), sólo si tenemos empleadoId
      let endpointUrl = null;
      try { endpointUrl = empleadoId ? (await getEmpleadoById(empleadoId))?.avatar || null : null; } catch {}

      const sources = [...localCandidates, endpointUrl, placeholderAvatar()].filter(Boolean);
      console.log("[Comentarios][Avatar] fuentes en orden:", sources);
      ...
      const tryNext = () => {
        ...
        const src = `${base}${srcNeedsBust(base) ? (base.includes("?") ? "" : `?v=${Date.now()}`) : ""}`;
        log("[Avatar][try]", src);
  ...
  async function loadComentarios(reqId) {
    try {
      ...
      // Prefetch empleados (IDs únicos) para acelerar endpointUrl cuando aplique
  ...
  function interceptComposer(reqId) {
    ...
    const send = async () => {
      ...
      if (!texto) return;
      if (!empleado_id) {
        err("[Comentarios][Send] sin empleado_id en sesión");
        return toast("No se encontró tu empleado en la sesión.", "danger");
      }
      btn.disabled = true;
      try {
        await createComentarioAPI({
          requerimiento_id: reqId,
          empleado_id,             // payload usa empleado_id (requerido por backend)
          comentario: texto,
          status: 1,
          created_by: empleado_id, // idem
        });
        ta.value = "";
  ...
  function resetTemplate() {
    // contacto
    $$('.exp-pane[data-tab="Contacto"] .exp-grid .exp-val').forEach((n) => {
      const a = n.querySelector("a");
      if (a) { a.textContent = "—"; a.removeAttribute("href"); }
      else n.textContent = "—";
    });
    // detalles
    $$('.exp-pane[data-tab="detalles"] .exp-grid .exp-val').forEach((n) => {
      if (n.id === "req-status") {
