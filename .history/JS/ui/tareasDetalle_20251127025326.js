...
export function createTaskDetailsModule({
  ...
  async function resolveAutorizaNombre(task) {
    ...
    if (task.autoriza_nombre_resuelta) {
      log("[KB] autoriza (cache tarea):", task.autoriza_nombre_resuelta);
      return task.autoriza_nombre_resuelta;
    }

    // 1) Intentar obtener departamento directo desde la tarea
    let departamentoId =
      task.departamento_id ??
      task.requerimiento_departamento_id ?? // por si viene así
      task.depto_id ??
      task.depto ??
      task.departamento ??
      null;

    log("[KB] autoriza → departamento en tarea:", departamentoId, "task.id:", task.id);

    // 2) Si no viene, lo sacamos del requerimiento asociado
    let req = null;
    if (!departamentoId && task.requerimiento_id) {
      try {
        ...
        log("[KB] autoriza → departamento desde req:", departamentoId, "req:", req);
      } catch (e) {
    ...
    if (!departamentoId) {
      warn("[KB] No se pudo determinar departamento para la tarea", task.id, task);
      return null;
    }

    // 3) Consultar departamento en backend
    try {
      const payload = {
        id: Number(departamentoId),
        // opcional, por si el backend lo requiere para regresar filas
        all: true,
      };
      ...
      if (nombreDirector) {
        ...
        task.autoriza_nombre_resuelta = nombreDirector;
        log("[KB] autoriza resuelto:", nombreDirector);
        return nombreDirector;
      }

      warn("[KB] DEPTO GET sin nombre de director para id:", departamentoId, d);
      return null;
    } catch (e) {
