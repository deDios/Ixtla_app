...
if ($path && file_exists($path)) {
  ...
  echo json_encode(["ok" => false, "error" => "No se encontró conexion.php"]);
  exit;
}
...
if (!$folio || !preg_match('/^REQ-\d{10}$/', $folio)) {
  ...
  echo json_encode(["ok" => false, "error" => "Folio inválido. Formato esperado: REQ-0000000000"]);
  exit;
}

if ($statusRaw === null || !preg_match('/^-?\d+$/', $statusRaw)) {
  http_response_code(400);
  echo json_encode(["ok" => false, "error" => "Status inválido. Debe ser un número entre 0 y 6."]);
  exit;
}
$status = (int)$statusRaw;
if ($status < 0 || $status > 6) {
  http_response_code(400);
  echo json_encode(["ok" => false, "error" => "Status inválido. Usa 0,1,2,3,4,5,6"]);
  exit;
}
...
if (empty($files)) {
  http_response_code(400);
  echo json_encode(["ok" => false, "error" => "No se recibieron archivos. Usa campo 'files' o 'file' (multipart/form-data)."]);
  exit;
}
