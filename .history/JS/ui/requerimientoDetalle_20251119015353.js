 ...
 * ========================= */
  ...
  async function fetchCCPByReqId(
    ...
    log("[CCP] fetchCCPByReqId → payload:", payload, "url:", ENDPOINTS.CCP_LIST);

    const res = await postJSON(ENDPOINTS.CCP_LIST, payload);

    // Aquí logueamos tal cual lo que regresa PHP
    log("[CCP] raw response desde endpoint:", res);

    if (!res) {
      warn("[CCP] respuesta vacía de endpoint");
      return null;
    }
    ...
    // 1) { ok, data: { ... } }
    // 2) { ok, data: [ ... ] }
    // 3) directamente { ... } (por si después lo cambias)
    let data = null;

    if (res.data != null) {
      data = res.data;
      log("[CCP] usando res.data:", data);
    } else {
      data = res;
      log("[CCP] usando res como objeto directo:", data);
    }

    // Si es arreglo, tomamos [0]
    if (Array.isArray(data)) {
      log("[CCP] data es Array, length:", data.length);
      return data[0] || null;
    }

    // Si es objeto plano
    if (data && typeof data === "object") {
      log("[CCP] data es objeto, keys:", Object.keys(data));
      return data;
    }

    warn("[CCP] formato inesperado en data:", data);
    return null;
  }
