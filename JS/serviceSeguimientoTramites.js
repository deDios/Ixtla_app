(() => {
  const ENDPOINT =
    ...
    IX_DEBUG_TRACK = true,
    CACHE_TTL_MS = 3e5;
  function depIdFromURL() {
    try {
      const s = new URLSearchParams(location.search),
        n = Number(s.get("depId"));
      return Number.isInteger(n) && n > 0 ? n : null;
    } catch {
      return null;
    }
  }
  const DEP_ID = depIdFromURL(),
    CACHE_KEY = `ix_req_cache_dept_${DEP_ID ?? "all"}`,
    NUM_STATUS_MAP = {
      0: "solicitud",
      1: "revision",
      2: "asignacion",
  ...
  const $ = (r, s) => r.querySelector(s),
    ...
    log = (...a) => {
      IX_DEBUG_TRACK && console.log("[IX][track]", ...a);
    },
    warn = (...a) => {
      IX_DEBUG_TRACK && console.warn("[IX][track]", ...a);
    },
    err = (...a) => {
      IX_DEBUG_TRACK && console.error("[IX][track]", ...a);
    };
  function withTimeout(pf, ms = FETCH_TIMEOUT) {
    const ctrl = new AbortController(),
      t0 = performance.now(),
      to = setTimeout(() => ctrl.abort(), ms);
    return pf(ctrl.signal)
      .then((r) => {
        clearTimeout(to);
        r.__ms = Math.round(performance.now() - t0);
        return r;
      })
      .catch((e) => {
        clearTimeout(to);
        e.__ms = Math.round(performance.now() - t0);
        throw e;
      });
  }
  ...
  function formatDateTime(s) {
    ...
    const meses = [
      ...
      date = `${dd} de ${meses[d.getMonth()]} ${d.getFullYear()}`;
    let h = d.getHours(),
      m = String(d.getMinutes()).padStart(2, "0");
    const ap = h >= 12 ? "pm" : "am";
    h = ((h + 11) % 12) + 1;
    return { date, time: `${h}:${m} ${ap}` };
  }
  ...
  function getCache(k) {
    try {
      ...
      if (!o || Date.now() - o.t > (o.ttl ?? 0)) return null;
      return o.v;
    } catch {
      return null;
    }
  }
  function setCache(k, v, ttl = CACHE_TTL_MS) {
    try {
      sessionStorage.setItem(k, JSON.stringify({ v, t: Date.now(), ttl }));
    } catch {}
  }
  const root = document.getElementById("tramites-busqueda");
  if (!root) return;
  const form =
      $("#tramites-busqueda", "#form-tramite") ||
      root.querySelector("#form-tramite"),
  ...
  function showPanel(w) {
    [pEmpty, pLoad, pError, pResult].forEach(
      (p) => p && p.classList.remove("is-visible")
    );
    w && ((w.hidden = false), w.classList.add("is-visible"));
  }
  function setLoading(on) {
    root.classList.toggle("is-loading", !!on);
    btnBuscar && (btnBuscar.disabled = !!on);
    inpFolio && (inpFolio.disabled = !!on);
  }
  function keyByStep(step) {
    return (
      {
        1: "solicitud",
        2: "revision",
  ...
  function setStepsActive(stepIndex) {
    ...
    const key = keyByStep(stepIndex);
    stepDescText && (stepDescText.textContent = MESSAGES[key] || "—");
  }
  ...
  function handleDocClick(e) {
    ...
    const isBtn = e.target.closest?.(".ix-stepbtn"),
      isPop = e.target.closest?.(".ix-pop");
    !isBtn && !isPop && closeAllPopovers();
  }
  function handleEsc(e) {
    "Escape" === e.key && closeAllPopovers();
  }
  ...
  function renderResult(row) {
    ...
    const dir = [row.contacto_calle, row.contacto_colonia, row.contacto_cp]
      .filter(Boolean)
      .join(", ");
    setText(elDir, dir || "—");
  ...
  const queryByFolio = (folio) =>
      withTimeout((sig) => fetchJSON({ folio }, sig)),
    queryList = (depId, opts = {}) => {
      const payload = {};
      Number.isInteger(depId) && (payload.departamento_id = depId);
      opts.all === true
        ? (payload.all = true)
        : ((payload.page = opts.page || 1),
          (payload.per_page = opts.per_page || 50));
  ...
  async function handleSubmit(e) {
    ...
    setLoading(true);
    showPanel(pLoad);
    log("Buscar folio:", folioNorm, "depId:", DEP_ID ?? "(todos)");
    try {
      const r1 = await queryByFolio(folioNorm);
      log(
        "Resp folio:",
        r1.http,
        r1.json?.ok ? "ok" : r1.json?.error,
        r1?.__ms ?? "?ms"
      );
      if (r1.json?.ok && r1.json?.data) {
        renderResult(r1.json.data);
        setLoading(false);
        return;
      }
    } catch (ex) {
      warn("Fallo puntual por folio, voy a listado:", ex?.message || ex);
    }
    try {
      let list = getCache(CACHE_KEY);
      if (!Array.isArray(list)) {
        const r2 = await queryList(DEP_ID, { all: true });
        log(
          "Resp listado:",
          r2.http,
          r2.json?.ok ? "ok" : r2.json?.error,
          r2?.__ms ?? "?ms"
        );
        if (!r2.json?.ok || !Array.isArray(r2.json?.data))
      ...
      const target = list.find(
        (row) =>
          String(row?.folio || "").toUpperCase() === folioNorm.toUpperCase()
      );
  ...
  $$(root, ".ix-pop-close").forEach((b) =>
    b.addEventListener("click", () => closeAllPopovers())
  );
