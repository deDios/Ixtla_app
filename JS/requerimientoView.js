...
(function () {
  ...
  const statusLabel = (s) => ({
    0:"Solicitud",1:"Revisión",2:"Asignación",3:"Proceso",4:"Pausado",5:"Cancelado",6:"Finalizado"
  }[Number(s)] || "—");

  const statusBadgeClass = (s) => ({
    0:"is-muted",1:"is-info",2:"is-info",3:"is-info",4:"is-warning",5:"is-danger",6:"is-success"
  }[Number(s)] || "is-info");
  ...
  async function createComentarioAPI({ requerimiento_id, usuario_id, empleado_id, comentario, status = 1 }) {
    ...
    if (!Number.isFinite(uid)) throw new Error("Falta id de usuario (created_by)");
    ...
    if (r?.ok === false) throw new Error(r?.error || "Error al crear comentario");
    return r;
  }
