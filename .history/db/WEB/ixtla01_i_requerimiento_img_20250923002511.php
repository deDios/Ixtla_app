...
if ($path && file_exists($path)) {include $path;} else {http_response_code(500);
    echo json_encode(["ok" => false, "error" => "No se encontró conexion.php"]);exit;}

/* ---------- Config ---------- */
$MAX_FILES    = 3;                           // max. archivos por request
$MAX_BYTES    = 30 * 1024 * 1024;            // 30 MB por archivo
$ALLOWED_MIME = ['image/jpeg', 'image/png']; // solo JPG/PNG)
...
if (isset($_FILES['files'])) {
    $files = array_merge($files, normalize_files_array($_FILES['files']));
}

if (isset($_FILES['file'])) {
    $files = array_merge($files, normalize_files_array($_FILES['file']));
}
...
if (! $webroot) {http_response_code(500);
    echo json_encode(["ok" => false, "error" => "No se pudo resolver webroot"]);exit;}
