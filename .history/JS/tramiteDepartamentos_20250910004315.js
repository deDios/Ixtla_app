(() => {
  // --- debug rapidito (pon en false si te estorba)
  ...
  // ===================== CONFIG QUE PUEDES TOCAR =====================
  // Pasos/estados del flujo (cámbialos si quieres otro naming u orden)
  window.IX_STEPS = window.IX_STEPS || ["Solicitud", "Revisión", "Asignación", "En proceso", "Finalizado"];

  // Cómo decides el estado actual:
  //  - 'map'  → usas IX_STATUS_BY_FOLIO (folio→índice)
  //  - 'fixed'→ usas IX_STATUS_FIXED_INDEX (mismo estado para todo)
  //  - 'seed' → automático en base al número del folio (default)
  window.IX_STATUS_MODE = window.IX_STATUS_MODE || "seed";

  // Mapa manual por folio (solo si IX_STATUS_MODE === 'map')
  // Ejemplo: { "ID00001": 2, "ID00077": 4 }
  window.IX_STATUS_BY_FOLIO = window.IX_STATUS_BY_FOLIO || {};

  // Índice fijo (0..IX_STEPS.length-1) si IX_STATUS_MODE === 'fixed'
  if (typeof window.IX_STATUS_FIXED_INDEX !== "number") window.IX_STATUS_FIXED_INDEX = null;

  // Auto avance (si lo prendes, va saltando de estado cada N ms y se reinicia)
  ...
  window.IX_AUTO_ADVANCE_RESET_TO = window.IX_AUTO_ADVANCE_RESET_TO || 0; // a qué índice reinicia cuando llega al final
  // ==================================================================

  // --- validación del folio (simple y clara)
  const FOLIO_RE = /^ID\d{5,}$/; // ID + 5 o más dígitos (ej: ID00001)
  const normalizeFolio = (s) => (s || "").toUpperCase().trim();

  // --- “semilla” para que la simulación sea estable por folio
  ...
  // --- decidir índice de estado según la config de arriba
  const clampIdx = (n) => {
