...
export function parseReq(raw) {
  ...
  // 1) Recolecta posibles fuentes del nombre
  const candFull = [
    ...
    typeof raw?.asignado                 === "string" ? raw.asignado.trim()                 : "", // a veces viene aquí en texto
  ].find(Boolean) || "";

  const candNombre = typeof raw?.asignado_nombre === "string" ? raw.asignado_nombre.trim() : "";
  const candApes   = typeof raw?.asignado_apellidos === "string" ? raw.asignado_apellidos.trim() : "";

  // 2) Si 'asignado' viene como string pero es claramente un número, no lo usamos como nombre
  const isOnlyDigits = (s) => /^\s*\d+\s*$/.test(String(s || ""));
  const fullFromApi = candFull && !isOnlyDigits(candFull) ? candFull : "";

  // 3) Normaliza nombre / apellidos
  let asignadoNombre = candNombre;
  let asignadoApellidos = candApes;

  if (!asignadoNombre && fullFromApi) {
    // Deriva nombre y apellidos del "full"
    const s = fullFromApi.replace(/\s+/g, " ").trim();
    const parts = s.split(" ");
    if (parts.length === 1) {
      asignadoNombre = parts[0];
    } else {
      asignadoNombre = parts.slice(0, -1).join(" ");
      asignadoApellidos = asignadoApellidos || parts.slice(-1).join(" ");
    }
  }
  ...
  // 4) Si seguimos sin nada legible, deja log para depurar
  if (!asignadoNombre && !fullFromApi) {
    // Solo logea en dev
    try {
      if (typeof window !== "undefined" && window?.location) {
        console.warn("[API:Requerimientos][parseReq] Sin nombre de asignado — id:", raw?.id, {
  ...
  return {
    ...
    // NUEVOS: expone 3 variantes
    asignadoNombre: asignadoNombre || (fullFromApi ? (fullFromApi.split(/\s+/).slice(0, -1).join(" ") || fullFromApi) : ""),
    asignadoApellidos: asignadoApellidos || "",
    asignadoFull: asignadoFull || "Sin asignar",

    // Compat: lo que Home usa por defecto → SOLO nombre (o full si no hubo split)
