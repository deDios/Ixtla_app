  ...
  async function areAllProcesosAndTasksDone(reqId) {
    try {
      ...
      for (const pr of procesos) {
        ...
        if (!tareas.length) return false; // proceso sin tareas => no está listo

        totalTareas += tareas.length;

        const todasHechas = tareas.every((ta) => {
          const est =
            Number(ta.estatus ?? ta.status ?? ta.estado ?? ta.estado_id ?? 0);
          const pct = Number(
            ta.porcentaje ??
          ...
          // 4 = HECHO (según tablero de tareas) o 100%
          return est === 4 || pct >= 100;
        });
      ...
      warn("[ReqView] areAllProcesosAndTasksDone error:", e);
      return false;
    }
  }
