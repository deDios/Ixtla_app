...
(() => {
  ...
  // Helpers (reusa los de RequerimientoView si existen)
  ...
  function addRow(table, item) {
    ...
    a.className = "exp-row";
    a.href  = item.url || "#";
    a.target = "_blank"; // abre en otra pestaña por si no es imagen
  ...
  function normalizeItem(r = {}) {
    // backend típico: { name, url, quien, fecha, tipo? } — cubrimos variantes
  ...
  /* ============ Modal de preview de imagen (opcional) ============ */
  ...
  function bindPreviewClicks() {
    ...
    table.addEventListener("click", (e) => {
      ...
      if (!/\.(png|jpe?g|gif|webp|bmp|heic|heif)$/i.test(href)) {
        // si no es imagen, deja que abra en otra pestaña
        return;
      }
      e.preventDefault();
      openPreview({
  ...
  /* ============ Boot ============ */
  function boot() {
    // Obtenemos el folio desde el DOM ya pintado por requerimientoView
    const folioNode = document.querySelector('.exp-meta > div:nth-child(1) dt')?.textContent?.toLowerCase().includes("contacto")
      ? document.querySelector('.exp-title h1') // fallback
      : null;
    // Mejor: intenta leer folio desde un atributo data-* si puedes inyectarlo
    // o toma el folio desde un lugar confiable del DOM:
    // (Si ya lo tienes en algún data-attr, cambia esto.)
    const folioTextNode = document.querySelector('[data-folio]'); // si existe <div data-folio="REQ-0000003623">
    const folio =
      folioTextNode?.getAttribute?.("data-folio") ||
      (window.__REQ__?.folio) ||                      // si algún script lo expone
    ...
    if (!folio) {
      ...
      // Aún así, bindea preview para las filas dummy
      bindPreviewClicks();
      return;
    }
