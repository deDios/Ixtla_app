...
$rol_id = isset($in['rol_id']) ? (int)$in['rol_id'] : (isset($in['id']) ? (int)$in['id'] : null);
$codigo = isset($in['codigo']) ? strtoupper(trim((string)$in['codigo'])) : '';
$activo = array_key_exists('activo', $in) ? (int)$in['activo'] : null;
$nivel_jerarquico = isset($in['nivel_jerarquico']) ? (int)$in['nivel_jerarquico'] : null;
...
if ($rol_id && $rol_id > 0) {
  ...
  echo json_encode(["ok" => true, "data" => rol_row($row)], JSON_UNESCAPED_UNICODE);
  exit;
}
...
if ($nivel_jerarquico !== null && $nivel_jerarquico > 0) {
