...
(function () {
  ...
  const SEL = {
    ...
    // Modal Nueva Tarea (ya existe en tu HTML)
    modalT:        "#modal-tarea",
    formT:         "#form-tarea",
    selProceso:    "#tarea-proceso",
    inpTitulo:     "#tarea-titulo",
    inpEsfuerzo:   "#tarea-esfuerzo",
    // (fechas removidas del modal)
    inpAsignado:   "#tarea-asignado",
    txtDesc:       "#tarea-desc",

    // Modal Nuevo Proceso (opcional; si no existe → prompt())
  ...
  // Fecha de hoy en formato DD/MM/YYYY (para la tabla)
  function todayMX() {
    const d = new Date();
  ...
  function bindProcessAccordion(sec) {
    ...
    head.addEventListener("click", (e) => {
      if (!e.target.closest(".chev")) return; // solo el chevron
    });
  ...
  function createProcess({ title, fechaInicio, porcentaje = 0 } = {}) {
    ...
    // Si el modal de tareas está abierto, refrescamos su select
    const modalT = document.querySelector(SEL.modalT);
    if (modalT && modalT.classList.contains("open")) fillProcesoSelect(id);
  ...
  function addTaskToProcess(procesoId, tarea) {
    ...
    row.innerHTML = `
      ...
      <div class="estatus"><span class="exp-badge is-info">Por hacer</span></div>
  ...
  function bindSubmitNuevaTarea() {
    ...
    form.addEventListener("submit", (e) => {
      ...
      const fechaI = todayMX();

      const ok = addTaskToProcess(procesoId, {
        titulo,
        esfuerzo,
        fecha_inicio: fechaI,   // ← se pinta en la tabla
        // fecha_fin: (se pondrá cuando la tarea pase a “Hecho”)
        asignado,
        descripcion: desc,
        porcentaje: 0,
        status: 1, // “Por hacer” (opcional)
      });
  ...
  // ===== Modal Nuevo Proceso (opcional) =====
  function openProcesoModal() {
    const modal = document.querySelector(SEL.modalP);
    const content = modal?.querySelector(".modal-content");
    if (!modal || !content) {
      // fallback sin modal
      const titulo = window.prompt("Título del nuevo proceso:");
      if (!titulo) return;
      const sec = createProcess({ title: titulo, porcentaje: 0 });
      if (sec) {
        toast("Se creó un nuevo proceso", "success");
    ...
    if (inpInicio) inpInicio.value = new Date().toISOString().slice(0,10);
  ...
  function bindSubmitNuevoProceso() {
    ...
    form.addEventListener("submit", (e) => {
      ...
      const inicio = $(SEL.inpPInicio)?.value || new Date().toISOString().slice(0,10);
      if (!titulo) { toast("Escribe un título para el proceso.", "warning"); $(SEL.inpPTitulo)?.focus(); return; }
      const sec = createProcess({ title: titulo, fechaInicio: inicio, porcentaje: 0 });
      if (sec) {
        toast("Se creó un nuevo proceso", "success");
        closeProcesoModal();
        sec.scrollIntoView({ behavior: "smooth", block: "start" });
  ...
  function bindToolbar() {
    ...
    btnProceso?.addEventListener("click", (e) => {
      ...
      openProcesoModal(); // sólo front-end
    });
    ...
    _boundToolbar = true;
    log("Toolbar enlazada (frontend-only)");
  }
