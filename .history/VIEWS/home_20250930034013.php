...
<body>
    ...
    <section aria-label="Panel de trámite">
        <aside class="ix-drawer" role="dialog" aria-modal="true" aria-labelledby="ix-drw-title" data-drawer="panel">
            ...
            <div class="ixd-body">
                ...
                <!-- Bloque imagen con overlay lápiz en modo edición -->
                ...
                <!-- Previews (locales) -->
                <div class="ixd-previews" data-img="previews" aria-live="polite"></div>
    ...
    <script type="module">
/* Hook ligero encima del Drawer existente */
(function () {
  ...
  function setEditing(on) {
    ...
    // toggle ver/editar por campo
    panel.querySelectorAll('[data-edit]').forEach(el => el.hidden = !on);
    panel.querySelectorAll('.ixd-field p').forEach(el => el.hidden = !!on);
    // botones
  ...
  function fillFields(row) {
    ...
    // evidencia “hero” (si la tienes guardada en tu dato, si no deja vacío)
    const hero = qs('[data-img="hero"]');
    if (hero) hero.src = row.hero_url || "";
  }

  // ---- Exponer to Drawer.open(raw, callbacks) ----
  const oldOpen = window.Drawer?.open;
  if (typeof oldOpen === "function") {
    window.Drawer.open = function (row, callbacks = {}) {
      oldOpen.call(window.Drawer, row, callbacks);
      // pinta y arranca en modo lectura
      fillFields(row);
      setEditing(false);
      filesBuf = [];
      qs('[data-img="previews"]').innerHTML = "";
      qs('[data-img="uploadBtn"]').disabled = true;

      // Guardar referencia del requerimiento actual
      panel._row = row;
  ...
  qs('[data-action="eliminar"]').addEventListener('click', async () => {
    ...
    try {
      const res = await fetch('/db/WEB/ixtla01_u_requerimiento.php', {
        method:'POST', headers:{'Content-Type':'application/json'},
        body: JSON.stringify({ id:r.id, status:0, updated_by:(window.__ixSession?.id_usuario ?? 1) })
      }).then(r=>r.json());
  ...
  // ---- Imagen: lápiz → input file + previews locales ----
  const fileInput = qs('[data-img="file"]');
  qs('[data-img="pick"]').addEventListener('click', () => {
    if (!panel.classList.contains('editing')) return; // solo en edición
    fileInput?.click();
  });
  ...
  fileInput.addEventListener('change', () => {
    ...
    list.forEach((f, i) => {
      ...
      card.querySelector('.rm').addEventListener('click', () => {
        filesBuf.splice(i,1);
        card.remove();
        qs('[data-img="uploadBtn"]').disabled = filesBuf.length===0;
      });
  ...
  // ---- Subir evidencia al status elegido ----
  qs('[data-img="uploadBtn"]').addEventListener('click', async () => {
    ...
    try {
      const up = await fetch('/db/WEB/ixtla01_c_requerimiento_media.php', { method:'POST', body: fd }).then(r=>r.json());
      if (!up?.ok) throw new Error(up?.error || 'Upload failed');
      // limpiar previews
      filesBuf = [];
      qs('[data-img="previews"]').innerHTML = "";
      qs('[data-img="uploadBtn"]').disabled = true;
      // si quieres refrescar galería remota aquí, llama a tu loader
    } catch (e) {
