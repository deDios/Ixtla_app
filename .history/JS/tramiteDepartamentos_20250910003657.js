(() => {
  // --- debug rapidito (true para ver logs)
  ...
  const panel = section.querySelector(".ix-result"); // acá vamos a pintar todo (aria-live="polite")

  if (!form || !input || !panel) { ixLog("Faltan refs del formulario/panel, me salgo."); return; }

  // --- estados del flujo (en el orden que deben verse en el stepper)
  const STEPS = ["Solicitud", "Revisión", "Asignación", "En proceso", "Finalizado"];

  // --- catálogo mini para “requerimiento” y direcciones fake (pura simulación)
  const REQS = [
