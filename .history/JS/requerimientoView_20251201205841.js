...
(function () {
  ...
  async function areAllProcesosAndTasksDone(reqId) {
    ...
    try {
      ...
      if (!procesos.length) {
        warn("areAllProcesosAndTasksDone() → sin procesos activos, devolviendo false");
        return false;
      }
      ...
      for (const pr of procesos) {
        ...
        log("areAllProcesosAndTasksDone() → TAREAS_LIST resp proceso", pr.id, t);
        ...
        if (!tareas.length) {
          warn(
            "areAllProcesosAndTasksDone() → proceso sin tareas activas, abortando con false",
            { proceso_id: pr.id }
          );
        ...
        const todasHechas = tareas.every((ta) => {
          ...
          const pct = Number(
            ta.porcentaje ??
              ...
              ta.progress ??
              ta.avance ??
              ta.porcentaje_avance_global ??
              0
          );

          // Flags adicionales que podrían venir del backend
          const doneFlag = ["hecho", "completado", "terminado"].some((k) => {
            if (!(k in ta)) return false;
            const v = ta[k];
            return (
              v === true ||
              v === 1 ||
          ...
          const isDone = est === 4 || pct >= 100 || doneFlag;
          ...
          log("areAllProcesosAndTasksDone() → tarea evaluada:", {
            ...
            estatus_raw: {
              ...
              status: ta.status,
              estado: ta.estado,
              estado_id: ta.estado_id,
              estatus_id: ta.estatus_id,
            },
            estatus_num: est,
            porcentaje_raw: {
              porcentaje: ta.porcentaje,
              porcentaje_avance: ta.porcentaje_avance,
              progress: ta.progress,
              avance: ta.avance,
              porcentaje_avance_global: ta.porcentaje_avance_global,
            },
            porcentaje_num: pct,
            doneFlag,
            isDone,
