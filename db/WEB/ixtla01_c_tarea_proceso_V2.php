<?php
$origin = $_SERVER['HTTP_ORIGIN'] ?? '';
$allowed = [
  'https://ixtla-app.com',
  'https://www.ixtla-app.com'
];

if (in_array($origin, $allowed, true)) {
  header("Access-Control-Allow-Origin: $origin");
  header("Access-Control-Allow-Credentials: true");
  header("Vary: Origin");
}

header("Access-Control-Allow-Methods: GET, POST, PUT, PATCH, DELETE, OPTIONS");
$reqHeaders = $_SERVER['HTTP_ACCESS_CONTROL_REQUEST_HEADERS'] ?? '';
if ($reqHeaders) {
  header("Access-Control-Allow-Headers: $reqHeaders");
} else {
  header("Access-Control-Allow-Headers: Content-Type, Authorization, X-Requested-With, Accept");
}

header("Access-Control-Max-Age: 86400");

if (($_SERVER['REQUEST_METHOD'] ?? '') === 'OPTIONS') {
  http_response_code(204);
  exit;
}


// Conexión DB
$path = realpath("/home/site/wwwroot/db/conn/conn_db.php");
if ($path && file_exists($path)) { include $path; }
else {
  http_response_code(500);
  die(json_encode(["ok"=>false,"error"=>"No se encontró conn_db.php en $path"]));
}

// Inputs
if ($method === 'GET') {
  $in = $_GET;
} else {
  $in = json_decode(file_get_contents("php://input"), true) ?? [];
}

$id          = isset($in['id']) ? (int)$in['id'] : null;
$proceso_id  = isset($in['proceso_id']) && $in['proceso_id'] !== '' ? (int)$in['proceso_id'] : null;
$asignado_a  = isset($in['asignado_a']) && $in['asignado_a'] !== '' ? (int)$in['asignado_a'] : null;
$status      = isset($in['status']) && $in['status'] !== '' ? (int)$in['status'] : null;
$date_from   = isset($in['date_from']) && trim($in['date_from'])!=='' ? trim($in['date_from']) : null;
$date_to     = isset($in['date_to'])   && trim($in['date_to'])  !=='' ? trim($in['date_to'])   : null;

$pageSize = isset($in['page_size']) ? max(1,min(200,(int)$in['page_size'])) : 10;

// Cursor (para infinite scroll)
$cursorUpdatedAt = null;
$cursorId = null;
if (isset($in['cursor']) && is_array($in['cursor'])) {
  $cursorUpdatedAt = isset($in['cursor']['updated_at']) && trim((string)$in['cursor']['updated_at']) !== ''
    ? trim((string)$in['cursor']['updated_at'])
    : null;
  $cursorId = isset($in['cursor']['id']) ? (int)$in['cursor']['id'] : null;
}

$con = conectar();
if (!$con) {
  http_response_code(500);
  echo json_encode(["ok"=>false,"error"=>"No se pudo conectar a la base de datos"]);
  exit;
}
$con->set_charset('utf8mb4');


// =====================
// 1) Consulta individual
// =====================
if ($id) {
  $sqlOne = "
    SELECT
      t.id,
      t.proceso_id,
      t.asignado_a,
      e.nombre AS asignado_nombre,
      e.apellidos AS asignado_apellidos,
      t.titulo,
      t.descripcion,
      t.esfuerzo,
      t.fecha_inicio,
      t.fecha_fin,
      t.status,
      t.created_at,
      t.updated_at,
      t.created_by,
      t.updated_by
    FROM tarea_proceso t
    LEFT JOIN empleado e ON e.id = t.asignado_a
    WHERE t.id = ?
    LIMIT 1
  ";

  $q1 = $con->prepare($sqlOne);
  if (!$q1) {
    http_response_code(500);
    echo json_encode(["ok"=>false,"error"=>"Error en prepare"]);
    $con->close();
    exit;
  }

  $q1->bind_param("i", $id);
  $q1->execute();
  $row = $q1->get_result()->fetch_assoc();
  $q1->close();
  $con->close();

  if (!$row) {
    http_response_code(404);
    echo json_encode(["ok"=>false,"error"=>"No encontrado"]);
    exit;
  }

  $row['id']          = (int)$row['id'];
  $row['proceso_id']  = (int)$row['proceso_id'];
  $row['asignado_a']  = $row['asignado_a'] !== null ? (int)$row['asignado_a'] : null;
  $row['esfuerzo']    = (int)$row['esfuerzo'];
  $row['status']      = (int)$row['status'];
  $row['created_by']  = $row['created_by'] !== null ? (int)$row['created_by'] : null;
  $row['updated_by']  = $row['updated_by'] !== null ? (int)$row['updated_by'] : null;
  $row['asignado_display'] = trim(($row['asignado_nombre'] ?? '').' '.($row['asignado_apellidos'] ?? ''));

  echo json_encode(["ok"=>true,"data"=>$row]);
  exit;
}


// =====================
// 2) Listado con cursor
// =====================
$where  = [];
$types  = "";
$params = [];

// Filtrar por proceso
if ($proceso_id !== null) {
  $where[]  = "t.proceso_id = ?";
  $types   .= "i";
  $params[] =& $proceso_id;
}

// Filtrar por responsable
if ($asignado_a !== null) {
  $where[]  = "t.asignado_a = ?";
  $types   .= "i";
  $params[] =& $asignado_a;
}

// Filtrar por status (1=activa, 0=baja lógica)
if ($status !== null) {
  $where[]  = "t.status = ?";
  $types   .= "i";
  $params[] =& $status;
}

// Rango de creación
if ($date_from !== null) {
  $where[]  = "t.created_at >= ?";
  $types   .= "s";
  $params[] =& $date_from;
}

if ($date_to !== null) {
  $where[]  = "t.created_at <= ?";
  $types   .= "s";
  $params[] =& $date_to;
}

/**
 * Orden para infinite scroll:
 * usamos "last_updated" = COALESCE(updated_at, created_at)
 * DESC para traer lo más reciente primero.
 */
$lastUpdatedExpr = "COALESCE(t.updated_at, t.created_at)";

// Cursor: traer “más abajo” (más viejo) que el cursor
if ($cursorUpdatedAt !== null && $cursorId !== null) {
  $where[] = "($lastUpdatedExpr < ? OR ($lastUpdatedExpr = ? AND t.id < ?))";
  $types  .= "ssi";
  $params[] =& $cursorUpdatedAt;
  $params[] =& $cursorUpdatedAt;
  $params[] =& $cursorId;
}

$limit = $pageSize + 1; // +1 para saber si hay más

$sql = "
  SELECT
    t.id,
    t.proceso_id,
    t.asignado_a,
    e.nombre AS asignado_nombre,
    e.apellidos AS asignado_apellidos,
    t.titulo,
    t.descripcion,
    t.esfuerzo,
    t.fecha_inicio,
    t.fecha_fin,
    t.status,
    t.created_at,
    t.updated_at,
    t.created_by,
    t.updated_by,
    $lastUpdatedExpr AS last_updated
  FROM tarea_proceso t
  LEFT JOIN empleado e ON e.id = t.asignado_a
";

if ($where) {
  $sql .= " WHERE ".implode(" AND ", $where);
}

$sql .= " ORDER BY $lastUpdatedExpr DESC, t.id DESC
          LIMIT ?";

$types   .= "i";
$params[] =& $limit;

$stmt = $con->prepare($sql);
if (!$stmt) {
  http_response_code(500);
  echo json_encode(["ok"=>false,"error"=>"Error en prepare"]);
  $con->close();
  exit;
}

call_user_func_array([$stmt,'bind_param'], array_merge([$types], $params));

$stmt->execute();
$rs = $stmt->get_result();

$rows = [];
while ($r = $rs->fetch_assoc()) {
  $r['id']          = (int)$r['id'];
  $r['proceso_id']  = (int)$r['proceso_id'];
  $r['asignado_a']  = $r['asignado_a'] !== null ? (int)$r['asignado_a'] : null;
  $r['esfuerzo']    = (int)$r['esfuerzo'];
  $r['status']      = (int)$r['status'];
  $r['created_by']  = $r['created_by'] !== null ? (int)$r['created_by'] : null;
  $r['updated_by']  = $r['updated_by'] !== null ? (int)$r['updated_by'] : null;

  $r['asignado_display'] = trim(($r['asignado_nombre'] ?? '').' '.($r['asignado_apellidos'] ?? ''));

  $rows[] = $r;
}
$stmt->close();
$con->close();

// has_more + next_cursor
$hasMore = false;
$nextCursor = null;

if (count($rows) > $pageSize) {
  $hasMore = true;
  // quitamos el extra
  $rows = array_slice($rows, 0, $pageSize);
}

if (!empty($rows)) {
  $last = $rows[count($rows)-1];
  $nextCursor = [
    "updated_at" => $last['last_updated'], // string "YYYY-mm-dd HH:ii:ss"
    "id" => (int)$last['id']
  ];
}

// (Opcional) si no quieres exponer last_updated al cliente, puedes quitarlo aquí.
// Yo lo dejo porque normalmente ayuda al JS para “days in column”.
echo json_encode([
  "ok" => true,
  "meta" => [
    "page_size" => $pageSize,
    "has_more" => $hasMore,
    "cursor" => $nextCursor
  ],
  "data" => $rows
]);
