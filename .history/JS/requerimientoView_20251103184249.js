...
import { getById as _getReqById } from "/JS/api/requerimientoDetalle.js";
...
const fmtDate  = (s="") => s ? s.replace("T"," ").replace(" 00:00:00"," 00:00") : "—";

const log     = (...a) => console.log("[Detalle]", ...a);
const logWarn = (...a) => console.warn("[Detalle]", ...a);
const logErr  = (...a) => console.error("[Detalle]", ...a);

/* =============== GET defensivo (algunos endpoints devuelven array) =============== */
async function getReqByIdDefensivo(id){
  console.groupCollapsed("[Requerimiento] getById", { id });
  try {
    const res = await _getReqById(id);
    let data = res?.data ?? res;
    console.log("→ respuesta cruda:", res);
    if (Array.isArray(data)) {
      console.log("↪ data es array, tomo el primero:", data[0]);
      data = data[0] || {};
    }
    console.groupEnd();
    return { ok: true, data };
...
function renderCommentsList(items = []) {
  console.groupCollapsed("[Comentarios][UI] render list");
  ...
  items.forEach(c => {
    const art = document.createElement("article");
    art.className = "msg";
    const nombre = firstTwo(c.nombre || c.empleado_nombre || c.autor || "Anónimo");
    const cuando = c.creado_at || c.created_at || c.fecha || "ahora";
    art.innerHTML = `
      <img class="avatar" src="/ASSETS/user/img_user1.png" alt="">
      <div>
    ...
    $(".text", art).textContent = c.comentario || c.texto || "";
    feed.appendChild(art);
  });
  const scroller = feed.parentElement || feed;
  scroller.scrollTo({ top: scroller.scrollHeight, behavior: "smooth" });
...
async function loadAndRenderComentarios(requerimiento_id) {
  console.groupCollapsed("[Comentarios] load/list", { requerimiento_id });
  try {
    const reqIdNum = Number(requerimiento_id);
    console.log("→ payload listComentarios:", { requerimiento_id: reqIdNum, page: 1, page_size: 100, status: 1 });
    const res = await listComentarios({ requerimiento_id: reqIdNum, page: 1, page_size: 100, status: 1 });
    console.log("← respuesta listComentarios:", res);

    const raw = res?.data ?? res?.items ?? res;
    const arr = Array.isArray(raw) ? raw : (Array.isArray(raw?.rows) ? raw.rows : []);
    console.log("→ array normalizado:", arr);

    // Filtrado defensivo por si el backend ignora el filtro
    ...
    if (filtered.length !== arr.length) {
      console.warn("⚠ API podría estar ignorando requerimiento_id. Filtré cliente.", { total: arr.length, filtrados: filtered.length });
    }
    ...
    logWarn("[Comentarios] list error:", e);
    renderCommentsList([]);
  } finally {
    console.groupEnd();
  }
...
function interceptComposer(requerimiento_id, empleado_id = null) {
  ...
  const send = async () => {
    ...
    console.groupCollapsed("[Comentarios] create", payload);
    try {
      btn.disabled = true;
      const res = await createComentario(payload);
      console.log("← respuesta createComentario:", res);
      ta.value = "";
      await loadAndRenderComentarios(requerimiento_id);
    } catch (e) {
      logErr("[Comentarios] create error:", e);
    } finally {
      btn.disabled = false;
      console.groupEnd();
    }
...
async function bootIntegracion() {
  console.groupCollapsed("[Boot] Detalle requerimiento");
  ...
  if (!reqId) {
    console.warn("No hay id en la URL (?id=XXXX). Se queda en modo demo.");
    console.groupEnd();
    return;
  }

  // (opcional) apaga el demo para no mezclar
  ...
  try {
    const { data: req } = await getReqByIdDefensivo(reqId);
    console.log("Requerimiento obtenido:", req);
    paintRequerimiento(req);
  } catch (e) {
    logWarn("Error consultando requerimiento:", e);
  }
