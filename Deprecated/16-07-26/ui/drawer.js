...
// 🔒 Blindaje: siempre adaptamos antes de pintar
function mapToFields(root, row = {}){
  const norm = adaptReq(row);
  $$("[data-field]", root).forEach(el => {
    const key = el.getAttribute("data-field");
    const fmt = KEY_FORMATTERS[key] || FMT_TEXT;
    el.textContent = fmt(norm[key], norm);
  });

  // Fix legado: si por alguna razón quedó un número, traducimos
  const pEst = $('[data-field="estatus"]', root);
  if (pEst) {
    const n = Number(pEst.textContent.trim());
    if (Number.isFinite(n) && STATUS_LABELS[n]) pEst.textContent = STATUS_LABELS[n];
  }
  const pPri = $('[data-field="prioridad"]', root);
  if (pPri) {
    const n = Number(pPri.textContent.trim());
    if (Number.isFinite(n) && PRIORIDAD_LABELS[n]) pPri.textContent = PRIORIDAD_LABELS[n];
  }

  const hidId = root.querySelector('input[name="id"][data-field="id"]');
  if(hidId && norm.id != null) hidId.value = norm.id;
}
