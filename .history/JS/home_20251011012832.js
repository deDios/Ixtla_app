...
function readCookieSession() {
  try {
    ...
    console.log(TAG, "cookie ix_emp:", json);
    return json;
  } catch (e) {
    console.warn(TAG, "no pude parsear cookie ix_emp:", e);
    return null;
  }
}
function readSession() {
  ...
  let s = null;
  try { s = window.Session?.get?.() || null; } catch {}
  // 2) fallback a cookie
  if (!s) s = readCookieSession();
  if (!s) return null;

  const idEmpleado = s.empleado_id ?? s.id_empleado ?? null;
  const depId      = s.departamento_id ?? null;

  return {
    idEmpleado: (idEmpleado != null ? Number(idEmpleado) : null),
    depId: (depId != null ? Number(depId) : null),
...
// Si falta empleado_id pero tenemos username, resolvemos con el endpoint
async function ensureViewerFromUsername(sess) {
  if (sess?.idEmpleado) return sess;
  if (!sess?.username) return sess;

  try {
    const url = "https://ixtlahuacan-fvasgmddcxd3gbc3.mexicocentral-01.azurewebsites.net/DB/WEB/ixtla01_c_empleado.php";
    const res = await fetch(url, {
      method: "POST",
      headers: { "Content-Type":"application/json", "Accept":"application/json" },
      body: JSON.stringify({ q: String(sess.username), page:1, page_size:5, status_empleado:1 })
    });
    const json = await res.json();
    const emp = (json?.data || []).find(e => e?.cuenta?.username === sess.username) || null;
    if (emp) {
      console.log(TAG, "viewer resuelto por username:", emp);
      return {
        ...sess,
        idEmpleado: Number(emp.id),
        depId: Number(emp.departamento_id ?? sess.depId ?? 1)
      };
    }
  } catch (e) {
    console.warn(TAG, "ensureViewerFromUsername() fallo:", e);
  }
  return sess;
}
  ...
  table = createTable({
    ...
    columns: [
      ...
      { key: "estatus", title: "Status", sortable: true, accessor: r => r.estatus?.code ?? "",
        render: (val) => {
          // Si tu CSS para "En proceso" usa "en_proceso" en lugar de "proceso", normalizamos aquí:
...
function wireSidebarEvents() {
  ...
  items.forEach(btn => {
    btn.addEventListener("click", () => {
      ...
      const labelEl = document.querySelector(SEL.tableStatusLabel);
      if (labelEl) labelEl.textContent = groupHumanLabel(S.filtros.status);
      runPipeline();
...
function runPipeline() {
  ...
  // Total del universo (no el filtrado), como definimos:
  const totalUniverso = S.items.length;
  const totalFiltrado = forTable.length;
  const totalEl = $(SEL.tableTotal);
  if (totalEl) totalEl.textContent = String(totalUniverso);
...
async function init() {
  ...
  if (!sess) {
    console.error(TAG, "No hay sesión (cookie/Session ausentes). ¿Se cargó /JS/auth/session.js antes que home.js?");
    return;
  }
  // Resolver por username si falta empleado_id
  sess = await ensureViewerFromUsername(sess);
  ...
  if (nameEl) nameEl.textContent = S.viewer.name || "—";
  const depBadge = $(SEL.profileBadge);
  if (depBadge && !depBadge.textContent) depBadge.textContent = "—";
  ...
  if (allBtn) {
    ...
    const labelEl = document.querySelector(SEL.tableStatusLabel);
    if (labelEl) labelEl.textContent = groupHumanLabel("todos");
  }
