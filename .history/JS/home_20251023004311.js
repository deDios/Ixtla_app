...
function filterRoleVisibility(items, { isAdmin, isPres, isDir, soyPL, isJefe, isAnal }) {
  if (isAdmin || isPres) return items; // ven todo

  // Director o Primera Línea: ocultar sólo 'solicitud' y 'revision' (sí ven 'asignacion')
  if (isDir || soyPL) {
    const hideForDir = new Set(["solicitud", "revision"]);
    return items.filter(r => !hideForDir.has(normalizeStatusKey(r?.estatus_key || r?.estatus || "")) &&
                              !hideForDir.has(normalizeStatusKey(r?.estatus?.key || "")));
  }

  // Jefe / Analista: ya vienen acotados por fuente (mío + equipo); no filtramos por estado
  if (isJefe || isAnal) return items;

  // Resto: ya vienen acotados por fuente
  return items;
}
...
async function loadScopeData() {
  ...
  // Visibilidad por rol (ocultar asignación/solicitud para Dir/PL/Jefe/Analista)
  items = filterRoleVisibility(items, { isAdmin, isPres, isDir, soyPL, isJefe, isAnal });
