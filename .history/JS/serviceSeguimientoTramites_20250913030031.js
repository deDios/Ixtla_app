...
// ← Mapeo CLAVE: pasos 1..5 tal como están en el HTML (data-step="1"…"5")
...
function setStepsActive(stepIndex){steps.forEach(li=>{const n=Number(li.getAttribute("data-step")||"0");li.classList.remove("done","current","pending");li.removeAttribute("aria-current");if(n<stepIndex)li.classList.add("done");else if(n===stepIndex){li.classList.add("current");li.setAttribute("aria-current","step")}else li.classList.add("pending")});const k=keyByStep(stepIndex); if(stepDescText) stepDescText.textContent=MESSAGES[k]||"—"}
...
function renderResult(row){
  ...
  const key=statusKeyFromRow(row), step=stepFromKey(key);
  setStepsActive(step);
  showPanel(pResult);
}
...
async function handleSubmit(e){
  ...
  }catch(ex){ warn("Fallo puntual por folio, iré a listado:",ex?.message||ex); }

  // 2) fallback por listado (sin caché)
  try{
