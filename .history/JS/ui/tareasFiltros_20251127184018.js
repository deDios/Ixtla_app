...
export function createTaskFiltersModule({
  ...
  function createMultiFilter(fieldEl, key, options) {
    ...
    function renderOptions() {
      ...
      options.forEach((opt) => {
        ...
        li.dataset.value = String(opt.value); function renderOptions() {
          if (!list) return;
          list.innerHTML = "";
          ...
          options.forEach((opt) => {
            ...
            // Contenedor izquierda: checkbox + label
            const left = document.createElement("div");
            left.className = "kb-multi-left";

            const cb = document.createElement("span");
            cb.className = "kb-multi-checkbox";

            const labelSpan = document.createElement("span");
            labelSpan.className = "kb-multi-label";
            labelSpan.textContent = opt.label;

            left.appendChild(cb);
            left.appendChild(labelSpan);

            // Contenedor derecha: contador de tareas
            const meta = document.createElement("span");
            meta.className = "kb-multi-meta";
            const countSpan = document.createElement("span");
            countSpan.className = "kb-multi-count";
            // Por defecto 0; luego updateAvailableOptions lo rellenará
  ...
  /* ========================================================================
   * Helper para reordenar opciones (Jira-like)
   * ======================================================================*/
