...
// Busca tel y última entrada (24h)
$st=$con->prepare("SELECT c.id, c.last_incoming_at, ct.wa_phone
                   FROM conversations c JOIN contacts ct ON ct.id=c.contact_id
                   WHERE c.id=?");
$st->bind_param('i',$conv_id); $st->execute();
...
// Llama al sender de texto libre (que espera `to`)
$payload = ["to"=>$row['wa_phone'], "text"=>["body"=>$text, "preview_url"=>false]];
$url = "https://ixtla-app.com/db/WEB/send_wapp_text.php";
...
$st=$con->prepare("INSERT INTO messages(conversation_id,direction,wa_message_id,msg_type,text,raw_json)
                   VALUES(?, 'out', ?, 'text', ?, ?)");
