// /JS/views/home.js
import { $, mountSkeletonList, toggle, fmtDateISOtoMX, escapeHtml } from "/JS/core/dom.js";
...
async function init() {
  ...
  // ===== Charts: montaje seguro =====
  const cy = $("#chart-year");
  const cm = $("#chart-month");
  if (!cy || !cm) {
    console.warn(`${TAG} No encontré uno de los canvas (#chart-year / #chart-month)`);
  }
  const lc = cy ? LineChart(cy) : null;
  const dc = cm ? DonutChart(cm) : null;
  lc?.mount({ data: [] });
  dc?.mount({ data: [] });
  ...
  // Oculta skeletons si tu CSS usa .is-loaded
...
function applyAndRenderTable(table) {
  ...
  // Búsqueda (folio, contacto, tel, dep, nombre de estatus)
  if (search) {
...
function computeSeriesAnual(rows = []) {
  // Cuenta por mes del año actual (0..11)
  const now = new Date();
...
// (Para server-side filtering en el futuro)
