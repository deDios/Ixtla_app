...
import { initDrawer } from "/JS/ui/drawer.js";
...
async function init() {
  ...
  // Delegación: abrir drawer al hacer click en una fila
  const tbody = document.querySelector("#tbl-wrap tbody");
  if (tbody) {
    tbody.addEventListener("click", (e) => {
      const tr = e.target.closest("tr");
      if (!tr) return;
      const rid = tr.dataset.id ? Number(tr.dataset.id) : null;
      let row = null;

      // Buscar por id si la tabla lo expuso; si no, intentar por folio (1a col)
      if (rid) {
        row = S.get().requerimientos.find(r => r.id === rid) || null;
      } else {
        const folioCell = tr.querySelector("td");
        const folio = folioCell?.textContent?.trim();
        row = S.get().requerimientos.find(r => r.folio === folio) || null;
      }
...
function applyAndRenderTable(table) {
  ...
  if (!rows.length) {
    ...
    table.setData(rows);
    toggle($("#tbl-empty"), false);
    $("#tbl-total").textContent = String(rows.length); // total filtrado
  }
