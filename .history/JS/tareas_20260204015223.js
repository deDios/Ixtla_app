...
function buildMoveOptionsForTask(task) {
  const oldStatus = Number(task?.status);
  if (!oldStatus) return [];

  // Si está locked por REQ pausado/cancelado → no se puede mover a nada
  if (isTaskLockedByReq(task)) return [];

  // Opciones permitidas: todo status donde canMoveTask() diga true
  // (incluye privilegiados automáticamente)
  const out = [];
